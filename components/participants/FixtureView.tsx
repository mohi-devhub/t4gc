    ...
    return (
      <Card className="p-12 text-center border-2 border-dashed bg-gradient-to-br from-blue-50 to-white">
        <div className="space-y-4">
          ...
          <h3 className="text-xl font-bold text-gray-800">No Fixtures Available</h3>
    ...
    <div className="space-y-6">
      ...
      {userTeam && relevantMatches.length > 0 && (
        <Card className="p-4 bg-blue-50 border-2 border-blue-600">
          <h3 className="font-bold text-blue-900 mb-2">Your Matches</h3>
                  ...
                  return (
                    <Card
                      ...
                      className={`p-4 hover:shadow-lg transition-all duration-300 border-l-4 ${
                        highlighted 
                          ? 'border-l-yellow-500 bg-gradient-to-br from-yellow-50 to-white shadow-md ring-2 ring-yellow-200' 
                          : 'border-l-green-500 bg-gradient-to-br from-white to-gray-50'
                      }`}
                    >
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <div className={`w-2 h-2 rounded-full ${highlighted ? 'bg-yellow-500' : 'bg-green-500'} animate-pulse`} />
                      </div>
                      <div className="space-y-2">
                        <div className="flex items-center gap-2">
                          <div className={`w-2 h-2 rounded-full ${match.team1 === userTeam ? 'bg-yellow-600' : 'bg-blue-600'}`} />
                          <span className={`font-semibold flex-1 ${match.team1 === userTeam ? 'text-yellow-900' : 'text-gray-800'}`}>
                            {match.team1}
                          </span>
                        </div>
                        ...
                        <div className="flex items-center gap-2">
                          <div className={`w-2 h-2 rounded-full ${match.team2 === userTeam ? 'bg-yellow-600' : 'bg-red-600'}`} />
                          <span className={`font-semibold flex-1 ${match.team2 === userTeam ? 'text-yellow-900' : 'text-gray-800'}`}>
                            {match.team2}
                          </span>
                        </div>
                      </div>
